...
import { getPlaceById, createPlace, deletePlace, getPlacesInViewport } from '../services/placeService';
...
export const getAllPlacesController = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const northeastPoint = JSON.parse(northeast as string);
    const southwestPoint = JSON.parse(southwest as string)
